# DESIGN-DNA — the SmoothUI-pattern design system, portable edition
...
> a direct port of SmoothUI (smoothui.dev). Copy this one file into the target
> repo and instruct the agent: *"Read DESIGN-DNA.md fully, then revamp this app
> to it."* Everything needed — tokens, recipes, anatomies, process — is below.
>
> **Provenance & license:** this language is adapted from
> [SmoothUI](https://smoothui.dev) by Edu Calvo, MIT-licensed. Any app adopting
> it must keep a credit line (see §13). Verified against the real SmoothUI
> source and battle-tested in the Varient port (build-certified).
...
four `--color-brand*` values.** (Varient uses ember `#FF5A1F`; SmoothUI's
original is pink `#FF64B1` / `#E3306D`.)
...
hex in component classNames (the two sanctioned exceptions SmoothUI itself
ships: the destructive button's red pair, §7).
...
Wherever this system ships, keep one line (footer + README):
