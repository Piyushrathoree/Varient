...
<ComponentPreview name="announcement-banner" variant="section" />
...
- **`brand`** — brand-tinted surface (`bg-brand/5`, brand-tinted border) with a brand dot indicator and brand-colored link text.

Pass `icon` to render a leading icon instead of the brand dot:
...
The banner is a `role="region"` labelled by the message paragraph. The dismiss control has an explicit `aria-label` (customizable via `dismissLabel`). Entrance and exit animations respect `prefers-reduced-motion`, and the dismiss button disables its press scale under reduced motion too.
