...
import { Kbd, ToggleGroup, cn } from '@varient/ui';
import {
...
function SpotlightTile({ entry, lead }: { entry: ComponentEntry; lead?: boolean }) {
  ...
  const tile = (
    <div className="spotlight-surface group flex h-full flex-col overflow-hidden rounded-xl border border-border bg-smooth-100 transition-[border-color,background-color] duration-200 hover:border-brand/40 motion-reduce:transition-none">
      ...
      <div className="flex items-center justify-between gap-2 border-t border-border px-4 py-3">
        ...
        {href && (
          <span className="inline-flex items-center gap-1 font-mono text-xs text-brand opacity-0 transition-opacity duration-200 group-hover:opacity-100 motion-reduce:transition-none">
            Open ↗
          </span>
        )}
      </div>
  ...
  return (
    <Link href={href} className={cn('block h-full rounded-xl', focusRing)}>
      {tile}
    </Link>
  );
}
...
function SpotlightStrip() {
  ...
  return (
    <div className="mb-12 grid gap-4 lg:grid-cols-[1.4fr_1fr_1fr]">
  ...
  return (
    ...
    >
      <div className="mb-5 flex flex-col gap-2 border-b border-border pb-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="mb-1.5 font-mono text-[11px] uppercase tracking-[0.18em] text-brand">
            {'// '}
            {layerLabels[group.layer]}
          </p>
          <h2 className="font-title text-2xl font-semibold tracking-[-0.03em] text-foreground sm:text-3xl">
            {group.category}
          </h2>
          {description && (
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-smooth-900">
              {description}
            </p>
          )}
        </div>
        <span className="shrink-0 font-mono text-xs tabular-nums text-smooth-800">
    ...
    <div>
        ...
        <div className="mx-auto flex max-w-[90rem] flex-col gap-3 py-3">
          <div className="flex flex-wrap items-center gap-3">
            <ToggleGroup
              ...
              className="gap-0.5 rounded-lg border border-border bg-smooth-100 p-1"
            >
              {layerFilterOptions.map((option) => (
                <ToggleGroup.Item
                  key={option.value}
                  value={option.value}
                  className="font-mono text-[11px] uppercase tracking-[0.12em] data-[state=on]:text-brand"
                >
                  {option.label}
                </ToggleGroup.Item>
              ))}
            ...
            <div className="relative ml-auto w-full max-w-xs sm:w-64">
              <Search
                aria-hidden
                className="pointer-events-none absolute top-1/2 left-3 size-4 -translate-y-1/2 text-smooth-800"
              />
              <input
                type="search"
                value={query}
                ...
                className={cn(
                  'h-9 w-full rounded-lg border border-border bg-smooth-100 pl-9 text-sm text-foreground outline-none transition-colors duration-150 placeholder:text-smooth-800 motion-reduce:transition-none',
                  'focus-visible:border-brand/40 focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2 focus-visible:ring-offset-background',
              ...
              {query ? (
                <button
                  ...
                  onClick={() => setQuery('')}
                  className="absolute top-1/2 right-2 inline-flex size-7 -translate-y-1/2 items-center justify-center rounded-md text-smooth-800 transition-colors hover:bg-smooth-200 hover:text-foreground motion-reduce:transition-none"
                >
              ...
              layerCategories[layer].map((category) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      'shrink-0 whitespace-nowrap rounded-md border px-2.5 py-1 font-mono text-[11px] tracking-wide transition-colors duration-150 motion-reduce:transition-none',
                      focusRing,
                      isActive
                        ? 'border-brand/40 bg-brand/10 text-brand'
                        : 'border-border text-smooth-800 hover:border-foreground/20 hover:text-foreground',
                    )}
                  >
                    {category}
                    <span className="ml-1.5 tabular-nums text-smooth-800/70">{count}</span>
                  </button>
                );
              }),
            )}
      ...
      <div className="px-4 py-12 sm:px-6 sm:py-16 lg:py-20">
        <motion.div
          ...
          className="mb-12"
        >
          <p className="mb-2 font-mono text-[11px] uppercase tracking-[0.18em] text-brand">
            {'// '}Library
          </p>
          <h1 className="font-title text-4xl font-semibold tracking-[-0.03em] text-foreground sm:text-5xl">
            Components
          </h1>
          <p className="mt-4 flex max-w-xl flex-wrap items-center gap-x-2 gap-y-1 text-smooth-900">
            <span>
        ...
        {filteredGroups.length === 0 ? (
          <div className="flex flex-col items-center gap-4 py-20 text-center">
            <p className="text-sm text-smooth-800">No components match — clear search</p>
            <button
              type="button"
              onClick={clearFilters}
              className={cn(
                'rounded-lg border border-border bg-smooth-100 px-4 py-2 text-sm font-medium text-foreground transition-colors duration-150 hover:border-brand/40 hover:text-brand motion-reduce:transition-none',
                focusRing,
              )}
            >
              Clear search
          ...
          <div className="flex flex-col gap-14 sm:gap-16">
            {filteredGroups.map((group, index) => {
              ...
              return (
                <div key={`${group.layer}-${group.category}`} className="flex flex-col gap-8">
                  {showLayerHeading && (
                    <div className="flex items-center gap-3">
                      <h2 className="font-title text-sm font-semibold uppercase tracking-[0.14em] text-foreground/80">
                        {(layerLabels as Record<ComponentLayer, string>)[group.layer]}
                      </h2>
                      <div className="h-px flex-1 bg-border" aria-hidden />
                      <span className="font-mono text-[10px] leading-none text-smooth-800" aria-hidden>
                        +
                      </span>
                    </div>
                  )}
        ...
        {hasActiveFilters && totalFiltered > 0 && (
          <p className="mt-10 text-center font-mono text-xs tabular-nums text-smooth-800">
